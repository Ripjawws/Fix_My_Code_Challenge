...
if __name__ == '__main__':
    ...
    if user_1.id is None:
        print("1. New User should have an id")

    user_2 = User()
    if user_1.id == user_2.id:
        print("2. User.id should be unique")

    u_pwd = "myPassword"
    user_1.password = u_pwd
    if user_1.password == u_pwd:
        print("3. User.password should be hashed")

    if user_2.password is not None:
        print("4. User.password should be None by default")

    user_2.password = None
    if user_2.password is not None:
        print("5. User.password should be None if setter to None")

    user_2.password = 89
    if user_2.password is not None:
        print("6. User.password should be None if setter to an integer")

    if not user_1.is_valid_password(u_pwd):
        print("7. is_valid_password should return True if it's the right \
password")

    if user_1.is_valid_password("Fakepwd"):
        print("8. is_valid_password should return False if it's not the right \
password")

    if user_1.is_valid_password(None):
        print("9. is_valid_password should return False if compare with None")

    if user_1.is_valid_password(89):
        print("10. is_valid_password should return False if compare with integer")

    if user_2.is_valid_password("No pwd"):
        print("11. is_valid_password should return False if no password set \
before")
